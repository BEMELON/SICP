...
; The result returned by makemonitored is a third procedure, say mf, 
...
(define (make-monitored func)
    (let ((count 0))
         (lambda (data)
             (define mf
                 (cond ((equal? data 'how-many-calls?) count)
                       ((equal? data 'reset-count) (begin (set! count 0) count))
                       (else (begin (set! count (+ count 1)) (func data)))))
            mf)))
